...
function productAvailableAtBranch(product, branchId = selectedDailyBranch) {
...
function addToCart(id) {
  ...
  const dailyLimit = productDailyLimit(product);
  if (product && cart[id] >= dailyLimit) return;
  cart[id] = (cart[id] || 0) + 1;
  saveCart();
...
function buildQuoteText() {
  ...
  const divider = "────────────────";
  let text = "🧾News Order\n\n";
  text += `Order Channel: ${selectedDailyBranch ? 'Daily branch order' : 'Event pre-order'}\n`;
  if (selectedDailyBranch) text += `Branch: ${DAILY_BRANCHES[selectedDailyBranch]?.name || selectedDailyBranch}\n`;
...
function updateSendButtonState() {
  ...
  const branchReady = !selectedDailyBranch || Boolean(DAILY_BRANCHES[selectedDailyBranch]);
  const complete = items.length > 0 && orderType && paymentMethod && name && isValidCambodiaPhone(phone) && addressOk && date && branchReady;
...
function buildReceiptHTML() {
  ...
  return `
    ...
    <div class="receiptBody">
      <div class="receiptRow"><span>Order</span><span>${selectedDailyBranch ? 'Daily branch order' : 'Event pre-order'}</span></div>
      ${selectedDailyBranch ? `<div class="receiptRow"><span>Branch</span><span>${escapeHTML(DAILY_BRANCHES[selectedDailyBranch]?.name || selectedDailyBranch)}</span></div>` : ''}
...
function buildLastOrderPreviewHTML(orderRecord = {}, historyIndex = 0) {
  ...
  const branchItem = items.find(item => item.order_channel === 'branch_daily' && DAILY_BRANCHES[item.branch_id]);
  const orderContext = branchItem
    ? `${orderType} · ${branchItem.branch_name || DAILY_BRANCHES[branchItem.branch_id].name}`
    : `${orderType} event pre-order`;
  ...
  return `
    <article class="lastOrderPreview">
      ...
      <span>
        <small>${escapeHTML(orderContext)}</small>
...
function buildOrderRecord(paymentVerifiedTranId = '', telegramSent = false) {
  ...
  const items = cartEntries().flatMap(([id, quantity]) => {
    ...
    return [{
      ...
      ...(selectedDailyBranch ? {
        order_channel: 'branch_daily',
        branch_id: selectedDailyBranch,
        branch_name: DAILY_BRANCHES[selectedDailyBranch]?.name || selectedDailyBranch
      } : { order_channel: 'event' }),
      quantity,
...
async function loadProducts() {
  ...
  allProducts = data || [];
  await Promise.all([loadMenuCategories(), loadBranchMenuAvailability(), loadBranchMenuQuantities()]);
  renderCategoryUI();
  renderAll(allProducts);
  initCardClicks();
  updateCartBar();
...
async function loadBranchMenuAvailability() {
  ...
  if (error) {
    console.warn('[L&K menu] Could not load branch availability:', error.message);
  ...
  try {
    ...
    console.warn('[L&K menu] Invalid branch availability setting:', parseError);
    branchMenuAvailability = {};
  }
}
...
async function loadBranchMenuQuantities() {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.warn('[L&K menu] Could not load daily quantities:', error.message);
...
function renderAll(products) {
  ...
  const visibleProducts = products.filter(product => (
    ...
    && productAvailableAtBranch(product)
  ));
...
function syncDailyOrderModeUI() {
  ...
  const chooserOverlay = document.getElementById('dailyBranchOrderOverlay');
  ...
  document.querySelectorAll('[data-daily-branch]').forEach(button => {
    const active = button.dataset.dailyBranch === selectedDailyBranch;
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', String(active));
  });
  document.documentElement.classList.toggle('dailyBranchSelected', Boolean(selectedDailyBranch));
}

function changeOrderMode(nextBranch = '') {
  if (nextBranch && !DAILY_BRANCHES[nextBranch]) return;
  if (nextBranch === selectedDailyBranch) {
    forceBranchChooser = false;
    history.replaceState(null, '', `${location.pathname}${location.hash}`);
    syncDailyOrderModeUI();
    document.getElementById('sec-bestseller')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    return;
  }
  if (cartEntries().length && !confirm('Changing order type will clear your current cart. Continue?')) return;
  if (cartEntries().length) clearCart();
  selectedDailyBranch = nextBranch;
  forceBranchChooser = false;
  history.replaceState(null, '', `${location.pathname}${location.hash}`);
  try {
    if (nextBranch) sessionStorage.setItem(ORDER_MODE_STORAGE_KEY, nextBranch);
    else sessionStorage.removeItem(ORDER_MODE_STORAGE_KEY);
  } catch { /* session storage may be unavailable */ }
  if (custDateInput && nextBranch) custDateInput.value = todayInputValue();
...
document.getElementById('dailyBranchOrder')?.addEventListener('click', event => {
  const button = event.target.closest('[data-daily-branch]');
...
function prefillCustomerFields() {
  ...
  if (custDateInput) custDateInput.value = selectedDailyBranch ? todayInputValue() : (saved.date || '');
...
function setupOrderPageLayout() {
  if (!IS_BRANCH_ORDER_PAGE) {
    selectedDailyBranch = '';
    syncDailyOrderModeUI();
    return;
  }

  const locationHeading = document.getElementById('sec-locations');
  const locations = document.querySelector('.locationsWrap');
  const dailyMode = document.getElementById('dailyOrderMode');
  const branchChooserOverlay = document.getElementById('dailyBranchOrderOverlay');
  const firstMenuSection = document.getElementById('sec-bestseller');
  if (locationHeading && locations && dailyMode && firstMenuSection) {
    if (branchChooserOverlay) document.body.append(branchChooserOverlay);
    // Keep the map and branch list after the menu. The branch chooser is a page-level modal.
    const hoursHeading = document.getElementById('sec-hours');
    (hoursHeading?.parentNode || firstMenuSection.parentNode)?.insertBefore(locationHeading, hoursHeading || null);
    locationHeading.insertAdjacentElement('afterend', locations);
  }
  syncDailyOrderModeUI();
}

/* ---------- go ---------- */
setupOrderPageLayout();
...
productsReady
  .then(async () => {
    if (IS_BRANCH_ORDER_PAGE) {
      try {
        const savedBranch = sessionStorage.getItem(ORDER_MODE_STORAGE_KEY) || '';
        if (DAILY_BRANCHES[savedBranch]) {
          selectedDailyBranch = savedBranch;
          renderAll(allProducts);
          initCardClicks();
          syncDailyOrderModeUI();
        }
...
supabase
  ...
  .on('postgres_changes', { event: '*', schema: 'public', table: 'site_settings' }, async payload => {
    ...
    } else if (settingKey === BRANCH_MENU_SETTING_KEY) {
      await loadBranchMenuAvailability();
      renderAll(allProducts);
      initCardClicks();
    } else if (settingKey === BRANCH_QUANTITY_SETTING_KEY) {
