...
function setupOrderPageLayout() {
  ...
  const branchChooser = document.getElementById('dailyBranchOrder');
  const firstMenuSection = document.getElementById('sec-bestseller');
  if (locationHeading && locations && dailyMode && firstMenuSection) {
    if (branchChooser) {
      locations.removeChild(branchChooser);
      dailyMode.insertAdjacentElement('afterend', branchChooser);
    }
    // Keep the map and branch list after the menu; only the branch chooser sits above it.
