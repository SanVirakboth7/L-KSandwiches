...
let orders = [];
let activeOrderChannel = 'event';
let branchMenuAvailability = {};
...
const DAILY_BRANCHES = {
  'branch-1': { label: 'Branch 1', customerLabel: 'ទីតាំងទី ១' },
  'branch-2': { label: 'Branch 2', customerLabel: 'ទីតាំងទី ២' },
  'branch-3': { label: 'Branch 3', customerLabel: 'ទីតាំងទី ៣' }
};
...
const dailySelectionList = document.getElementById('dailySelectionList');
...
async function loadBranchMenuQuantities() {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.warn('[L&K admin] Could not load daily quantities:', error.message);
    branchMenuQuantities = {};
    return;
  }
  try {
    const parsed = data?.value ? JSON.parse(data.value) : {};
    branchMenuQuantities = parsed && typeof parsed === 'object' ? parsed : {};
  } catch {
    branchMenuQuantities = {};
  }
}
    ...
    ${items.map(product => {
      ...
      return `<div class="dailySelectionRow${offered ? ' is-selected' : ''}" data-daily-row-id="${escapeAttr(product.id)}">
        ...
        <span class="dailySelectionControls">
          ...
          <input class="dailyQuantityInput" type="number" min="0" max="999" step="1" value="${Math.max(0, Number(quantityMap[product.id]) || 0)}" data-daily-quantity-id="${escapeAttr(product.id)}" aria-label="${escapeAttr(product.name || product.id)} quantity" ${offered ? '' : 'disabled'}>
        </span>
      </div>`;
    }).join('')}
...
function openDailySelection() {
  if (!dailySelectionOverlay) return;
  loadBranchMenuQuantities().then(() => {
    ...
    dailySelectionOverlay.classList.add('open');
    dailySelectionOverlay.setAttribute('aria-hidden', 'false');
  });
}

function closeDailySelection() {
  dailySelectionOverlay?.classList.remove('open');
  dailySelectionOverlay?.setAttribute('aria-hidden', 'true');
}

document.getElementById('quickDailySelection')?.addEventListener('click', openDailySelection);
dailySelectionClose?.addEventListener('click', closeDailySelection);
dailySelectionOverlay?.addEventListener('click', event => {
  if (event.target === dailySelectionOverlay) closeDailySelection();
});
dailySelectionTabs?.addEventListener('click', event => {
...
dailySelectionList?.addEventListener('change', event => {
  ...
  if (toggle) {
    ...
    if (toggle.checked) {
      ...
      if (input) { input.disabled = false; input.value = String(Math.max(0, Number(quantityMap[productId]) || 0)); }
...
dailySelectionSave?.addEventListener('click', async () => {
  ...
  dailySelectionList.querySelectorAll('[data-daily-quantity-id]').forEach(input => {
    ...
    if (toggle?.checked) nextQuantities[input.dataset.dailyQuantityId] = Math.max(0, Math.min(999, Number.parseInt(input.value, 10) || 0));
  });
  ...
  const { error } = await supabase.from('site_settings').upsert({
    ...
    value: JSON.stringify(branchMenuQuantities)
  }, { onConflict: 'key' });
  dailySelectionSave.disabled = false;
  if (error) {
    if (dailySelectionStatus) dailySelectionStatus.textContent = 'Could not save';
...
const ordersBranchFilters = document.getElementById('ordersBranchFilters');
...
function orderChannelMeta(order) {
  const branchItem = safeOrderItems(order.items).find(item => item.order_channel === 'branch_daily' && DAILY_BRANCHES[item.branch_id]);
  if (branchItem) {
    return {
      channel: branchItem.branch_id,
      label: branchItem.branch_name || DAILY_BRANCHES[branchItem.branch_id].customerLabel,
      isDaily: true
    };
  }
...
function orderCardHTML(order) {
  ...
  const contactActionsHTML = normalizedPhone
    ? `<div class="adminOrderContactActions" aria-label="Contact customer">
        ...
        <a class="adminOrderContactBtn adminOrderTelegramBtn" href="${escapeAttr(telegramHref)}" target="_blank" rel="noopener" aria-label="Message ${escapeAttr(String(order.customer_name || 'customer'))} on Telegram" title="Open Telegram">
    ...
    <article class="adminOrderCard${orderCardClass}" data-order-id="${escapeAttr(String(order.id || ''))}" data-order-label="Order #${escapeAttr(String(orderNumber))}">
      ...
      <div class="adminOrderBody">
        ...
        <div class="adminOrderDetailsSource" hidden>
          ${(order.order_type === 'delivery' || orderNotes) ? `<section class="adminOrderPopupSection">
            <span class="adminOrderPopupLabel">Delivery details</span>
          ...
          ${order.payment_transaction_id ? `<section class="adminOrderPopupSection adminOrderPaymentReference">
            <span class="adminOrderPopupLabel">Payment reference</span>
            <p class="adminOrderTransaction">${escapeHTML(String(order.payment_transaction_id))}</p>
          </section>` : ''}
        </div>
      </div>

      <div class="adminOrderActions" aria-label="Update order status">
        <button type="button" class="adminOrderCompleteBtn adminOrderCompleteSlider${orderStatus === 'completed' ? ' is-active' : ''}" data-order-status="completed" aria-label="${orderStatus === 'completed' ? 'Order completed' : 'Slide to complete order'}"${orderStatus === 'completed' ? ' disabled' : ''}>
          <span class="adminOrderCompleteThumb" aria-hidden="true">
          ...
          <span class="adminOrderCompleteText">${orderStatus === 'completed' ? 'Completed' : 'Slide to complete'}</span>
        </button>
        <button type="button" class="adminOrderActionBtn adminOrderCancelBtn${orderStatus === 'cancelled' ? ' is-active' : ''}" data-order-status="cancelled"${orderStatus === 'cancelled' ? ' disabled' : ''}>
          <span>${orderStatus === 'cancelled' ? 'Cancelled' : 'Cancel'}</span>
        </button>
      </div>
    </article>`;
}
...
function renderOrders() {
  ...
  const visibleOrders = activeOrderChannel === 'all'
    ? orders
    : orders.filter(order => orderChannelMeta(order).channel === activeOrderChannel);
  ordersCount.textContent = `${visibleOrders.length} order${visibleOrders.length === 1 ? '' : 's'} · ${formatOrderPeriodLabel()}`;
  ordersList.innerHTML = visibleOrders.length
    ? visibleOrders.map(orderCardHTML).join('<div class="ordersListDivider" aria-hidden="true"></div>')
...
function startCompleteSlide(event, slider) {
  if (slider.disabled || event.button > 0) return;
  event.preventDefault();
  const card = slider.closest('.adminOrderCard');
  if (!card) return;

  const rect = slider.getBoundingClientRect();
  const minLeft = 4;
  const maxLeft = Math.max(minLeft, rect.width - 33);
  const startX = event.clientX;
  let progress = 0;
  slider.style.setProperty('--complete-progress', '0%');
  slider.classList.add('is-dragging');
  slider.setPointerCapture?.(event.pointerId);

  const move = moveEvent => {
    progress = Math.max(0, Math.min(1, (moveEvent.clientX - startX) / Math.max(1, maxLeft - minLeft)));
    slider.style.setProperty('--complete-thumb-left', `${minLeft + ((maxLeft - minLeft) * progress)}px`);
    slider.style.setProperty('--complete-progress', `${Math.round(progress * 100)}%`);
  };
  ...
  const finish = finishEvent => {
    ...
    if (slider.hasPointerCapture?.(finishEvent.pointerId)) slider.releasePointerCapture(finishEvent.pointerId);
    slider.classList.remove('is-dragging');
    if (progress >= 0.72) {
      slider.style.setProperty('--complete-thumb-left', `${maxLeft}px`);
      slider.style.setProperty('--complete-progress', '100%');
      slider.querySelector('.adminOrderCompleteText')?.replaceChildren(document.createTextNode('Completing…'));
      updateOrderStatus(card.dataset.orderId, 'completed', slider);
      return;
    }
    slider.style.removeProperty('--complete-thumb-left');
    slider.style.removeProperty('--complete-progress');
  };
...
ordersBranchFilters?.addEventListener('click', event => {
  const button = event.target.closest('[data-order-channel]');
  if (!button) return;
  activeOrderChannel = button.dataset.orderChannel || 'all';
  ordersBranchFilters.querySelectorAll('[data-order-channel]').forEach(item => {
...
ordersList?.addEventListener('click', event => {
  ...
  if (statusButton) {
    if (statusButton.matches('.adminOrderCompleteSlider')) return;
...
ordersList?.addEventListener('pointerdown', event => {
  const slider = event.target.closest('.adminOrderCompleteSlider');
  if (slider) startCompleteSlide(event, slider);
});
ordersList?.addEventListener('keydown', event => {
  const slider = event.target.closest('.adminOrderCompleteSlider');
  if (!slider || !['Enter', ' '].includes(event.key)) return;
  event.preventDefault();
  const card = slider.closest('.adminOrderCard');
  if (card) updateOrderStatus(card.dataset.orderId, 'completed', slider);
});
...
async function loadProducts() {
  ...
  products = data || [];
  await loadBranchMenuAvailability();
...
async function loadBranchMenuAvailability() {
  const { data, error } = await supabase
    .from('site_settings')
    .select('value')
    .eq('key', BRANCH_MENU_SETTING_KEY)
    .maybeSingle();
  if (error) {
    console.warn('[L&K admin] Could not load branch menu availability:', error.message);
    branchMenuAvailability = {};
    return;
  }
  try {
    const parsed = data?.value ? JSON.parse(data.value) : {};
    branchMenuAvailability = parsed && typeof parsed === 'object' ? parsed : {};
  } catch {
    branchMenuAvailability = {};
  }
}

function isProductAvailableAtBranch(productId, branchId) {
  return branchMenuAvailability?.[branchId]?.[productId] !== false;
}

async function saveProductBranchAvailability(productId, branchId, isAvailable, button) {
  branchMenuAvailability[branchId] ||= {};
  branchMenuAvailability[branchId][productId] = isAvailable;
  button?.classList.add('saving');
  const { error } = await supabase.from('site_settings').upsert({
    key: BRANCH_MENU_SETTING_KEY,
    value: JSON.stringify(branchMenuAvailability)
  }, { onConflict: 'key' });
  button?.classList.remove('saving');
  if (error) {
    branchMenuAvailability[branchId][productId] = !isAvailable;
    button?.classList.toggle('active', !isAvailable);
    button?.setAttribute('aria-pressed', String(!isAvailable));
    toast('Could not save branch availability: ' + error.message, true);
    return;
  }
  toast(`${productId} ${isAvailable ? 'available' : 'hidden'} at ${DAILY_BRANCHES[branchId].label}`);
}
...
function wireRow(id) {
  ...
  const branchButtons = row.querySelectorAll('[data-role="branchAvailability"]');
  ...
  branchButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      button.title = `${next ? 'Available' : 'Hidden'} at ${DAILY_BRANCHES[branchId].label}`;
      saveProductBranchAvailability(id, branchId, next, button);
    });
  });
}
